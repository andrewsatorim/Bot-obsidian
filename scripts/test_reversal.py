"""Test Reversal Strategy on top assets.

Catches major trend reversals with trailing stop, no TP.

Usage:
    python scripts/test_reversal.py
"""
...
def run_backtest(bundles, symbol):
    ...
    engine = BacktestEngine(
        ...
        strategy=ReversalStrategy(symbol=symbol),
...
def main():
    ...
    if results:
        ...
        # --- EXIT STATS ---
        print(f"\n{'='*110}")
        print(f"СТАТИСТИКА ВЫХОДОВ (Trailing Stop 2.5 ATR, без TP)")
        print(f"{'='*110}")
        total_sl = 0
        total_timeout = 0
        for name in sorted(results):
            ...
            print(f"  {name:<20} TrailingStop={r.sl_hits}  Timeout={r.timeout_exits}")
        print(f"{'-'*110}")
        print(f"  {'ИТОГО':<20} TrailingStop={total_sl}  Timeout={total_timeout}")
        print(f"{'='*110}")
