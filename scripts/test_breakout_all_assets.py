"""Test Breakout on ALL asset classes available on OKX.

Crypto + Gold + Silver + Forex + Equity perpetuals.

Usage:
    python scripts/test_breakout_all_assets.py
"""
...
def check_instrument_exists(inst_id):
    """Check if instrument exists on OKX."""
    try:
...
def main():
    ...
    for name in sorted(results, key=lambda x: results[x].total_return_pct, reverse=True):
        ...
        by_class[cls].append(r.total_return_pct)
    ...
    # Per-class summary
    print(f"\nBy asset class:")
    for cls, rets in sorted(by_class.items()):
        avg_r = sum(rets) / len(rets)
        prof = sum(1 for r in rets if r > 0)
        print(f"  {cls:<10}: avg {avg_r:+.2f}%, profitable {prof}/{len(rets)}")
