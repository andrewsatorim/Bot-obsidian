"""Test Elliott Wave Strategy — multi-TF wave detection.

Usage:
    python scripts/test_elliott.py
"""
...
def run_backtest(bundles, symbol):
    ...
    settings = Settings(account_equity=10_000.0, paper_trading=True, max_position_pct=0.25)
    engine = BacktestEngine(
        analytics=FeatureEngine(),
        strategy=ElliottWaveStrategy(symbol=symbol, wave_lookback=40, cooldown_bars=25),
        risk=RiskManager(settings),
        initial_equity=10_000.0,
        leverage=25,
        max_position_pct=0.25,
        atr_risk_multiplier=1.5,
        tp_levels=[],              # NO TP — trailing stop
...
def main():
    print(f"{'='*110}")
    print(f"ELLIOTT WAVE STRATEGY")
    print(f"Wave structure detection + Volume confirm + Liquidation zones")
    print(f"4H structure -> 1H/30m confirm -> 15m/5m entry (simulated via pivot order)")
    print(f"Leverage: 25x | Margin: 25% | SL: 1.5 ATR | Trailing: 2.5 ATR | NO TP")
    print(f"{'='*110}")

    print("\nChecking instruments...", end=" ", flush=True)
    valid = []
    for okx_id, ccxt_id, cls in SYMBOLS:
        if check_instrument_exists(okx_id):
            valid.append((okx_id, ccxt_id, cls))
            print(okx_id.split("-")[0], end=" ", flush=True)
        time.sleep(0.2)
    print(f"\n{len(valid)} available\n")

    results = {}
    for okx_id, ccxt_id, cls in valid:
    ...
    # Summary
    print(f"\n{'='*110}")
    print(f"{'Asset':<20} {'Class':<8} {'Trades':>6} {'WR':>6} {'PF':>7} {'Return':>8} {'MDD':>7} {'Equity':>10}")
    print(f"{'-'*110}")
    total_ret = 0
    profitable = 0
    for name in sorted(results, key=lambda x: results[x].total_return_pct, reverse=True):
        ...
        pf = f"{r.profit_factor:.2f}" if r.profit_factor < 100 else "inf"
        print(f"{name:<20} {cls:<8} {r.total_trades:>6} {r.win_rate:>5.1%} {pf:>7} {r.total_return_pct:>+7.2f}% {r.max_drawdown_pct:>6.2f}% ${r.final_equity:>9.2f}")
    ...
    if results:
        ...
        for name in sorted(results, key=lambda x: results[x].total_return_pct, reverse=True):
            ...
            for j, t in enumerate(r.trades):
                ...
                dur = f"{days:.1f}д" if days >= 1 else f"{hours:.1f}ч"
                print(f"    #{j+1} {t.direction.value:>5} {t.entry_price:.2f}->{t.exit_price:.2f} "
                      f"PnL={pnl_sign}{t.pnl:.2f} длит={dur}")

        # Profit
        deposit = 10_000.0
        print(f"\n{'='*110}")
        print(f"ПРОФИТ НА ${deposit:,.0f}")
        print(f"{'='*110}")
        for name in sorted(results, key=lambda x: results[x].total_return_pct, reverse=True):
            r = results[name]
            profit = deposit * r.total_return_pct / 100
            print(f"  {name:<20} {r.total_return_pct:>+7.2f}%  ->  {'+'if profit>=0 else ''}{profit:.2f}")
        print(f"{'-'*110}")
        print(f"  {'СРЕДНИЙ':<20} {avg:>+7.2f}%  ->  +{deposit*avg/100:.2f}")
